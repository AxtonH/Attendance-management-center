...
The route layer subtracts the on-leave set from each day's working
(in-office) set via `exclude_on_leave`, then hands that reduced set to the
existing builders as `working_emp_codes`. So these tests verify two things:

  1. `exclude_on_leave` does the subtraction and preserves the phase-1
     `None` sentinel.
  2. Feeding a leave-reduced working set to `build_overview` /
     `build_exceptions` / `build_departments_rollup` drops the on-leave
...
from app.infra.roster import exclude_on_leave
...
class TestExcludeOnLeaveHelper:
    def test_subtracts_leave_from_working(self):
        working = frozenset({"1001", "1002", "1003"})
        assert exclude_on_leave(working, frozenset({"1002"})) == frozenset(
            {"1001", "1003"}
        )

    def test_none_working_preserved(self):
        # Phase-1 sentinel must survive untouched.
        assert exclude_on_leave(None, frozenset({"1002"})) is None

    def test_empty_or_none_leave_is_noop(self):
        working = frozenset({"1001"})
        assert exclude_on_leave(working, None) == working
        assert exclude_on_leave(working, frozenset()) == working
...
class TestAbsentExcludesOnLeave:
    def test_overview_absent_drops_on_leave_person(self):
        ...
        working = exclude_on_leave(ROSTER, frozenset({"1002"}))
